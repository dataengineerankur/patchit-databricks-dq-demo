...
if missing_orders or missing_finance or missing_inventory:
    raise ValueError(
        "Upstream contract drift detected for complex margin flow: "
        f"orders_missing={missing_orders}, finance_missing={missing_finance}, inventory_missing={missing_inventory}"
    )
...
missing_inventory_count = margin_df.filter(F.col("on_hand").isNull()).count()
missing_ts = margin_df.filter(F.col("order_ts").isNull()).count()

if missing_fx > 0 or missing_inventory_count > 0 or missing_ts > 0:
    print(
        f"[COMPLEX] Filling nulls with defaults - "
        f"missing_fx={missing_fx}, missing_inventory={missing_inventory_count}, missing_order_ts={missing_ts}"
    )

margin_df = (
    margin_df
    .withColumn("fx_rate", F.coalesce(F.col("fx_rate"), F.lit(1.0)))
    .withColumn("on_hand", F.coalesce(F.col("on_hand"), F.lit(0)))
